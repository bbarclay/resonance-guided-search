...
def adaptability(x: float, d_res: float, N_ord: List[int], x0: float = 0.0) -> float:
    ...
    Raises:
        ValueError: If N_ord is an empty list.
    """
    if not N_ord:
...
def modulating_function(
    A1: float, A2: float, w: float = 2.0, epsilon: float = 1e-9
) -> float:
    """
...
def adaptability_array(
    x_array: np.ndarray, d_res: float, N_ord: List[int], x0: float = 0.0
) -> np.ndarray:
    """
